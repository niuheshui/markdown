...
class Solution:
    ...
    def dfs(self, r1, r2):
        ...
        if not r1 or not r2:
            return False
        return r1.val == r2.val and self.dfs(r1.left, r2.right) and self.dfs(r1.right, r2.left)
    ...
    public boolean isValid(String s) {
        ...
        for (char ch : s.toCharArray()) {
            if (map.containsKey(ch)) {
                if (sk.isEmpty() || sk.peek() != map.get(ch))
                    return false;
                sk.pop();
            } else {
                sk.push(ch);
...
class MinStack {
    ...
    Deque<Integer> sk, minsk;

    public MinStack() {
        sk = new LinkedList<>();      // 存储值
        minsk = new LinkedList<>();   // 存储目前最小值
    }
    
    public void push(int val) {
        sk.push(val);
        if (minsk.isEmpty() || minsk.peek() >= val)
            minsk.push(val);
    }
...
class Solution {
    private int[] getCount(String s, int start) {
        int x = 0;
        char ch;
        while ((ch = s.charAt(start++)) >= '0' && ch <= '9') {
            x = x * 10 + (ch - '0');
        }
        return new int[] {x, start};
    }

    private void add(StringBuffer sb, int count, String s) {
        for (int i = 0; i < count; i++)
            sb.append(s);
    }

    private int getNext(String s, int p) {
        int sk = -1;
        while (true) {
            char c = s.charAt(p);
            if (c == '[') sk--;
    ...
    public String decodeString(String s) {
        ...
        while (i < s.length()) {
            ...
            // 如果是数字获取数字
            if (ch >= '0' && ch <= '9') {
                int[] res = getCount(s, i);
                int start = res[1];
                int count = res[0];
                String ns = decodeString(s.substring(start));
                i = getNext(s, start);
                add(sb, count, ns);
            } else if (ch >= 'a' && ch <= 'z') {
                sb.append(ch);
...
```python
class Solution:
    def dailyTemperatures(self, C: List[int]) -> List[int]:
		n = len(C)
		ans = [0] * n
		stack = []
...
class Solution {
    ...
    private void sort(int[] arr, int l, int r, int k) {
        ...
        swap(arr, l, last);
        last = l;
        for (int i = l + 1; i <= r; i++) if (arr[i] > arr[l]) 
            swap(arr, i, ++last);
        swap(arr, l, last);
        if (last >= k) sort(arr, l, last - 1, k);
        if (last < k) sort(arr, last + 1, r, k);
    }
...
class Solution:
    ...
    def findKthLargest(self, nums: List[int], k: int) -> int:
        ...
        for i in range(n - 1, len(nums) - k, -1):
...
class Solution {
    ...
    private int val(int idx) {
        return map.get(heap[idx]);
    }

    // 上浮节点
    private void up(int i) {
        if (i < 1 || val((i-1)/2) <= val(i))
            return;
        swap(i, (i-1)/2);
        up((i-1)/2);
    }
    ...
    private void down(int i) {
        int l = i * 2 + 1, r = i * 2 + 2;
        int next = i;
        if (l < size && val(l) < val(next)) 
            next = l;
        if (r < size && val(r) < val(next))
            next = r;
        if (next == i)
            return;
        swap(next, i);
    ...
    public int[] topKFrequent(int[] nums, int k) {
        ...
        // 堆中元素小于k个就插入, 因为是小根堆, 堆顶是最小的元素, 大于就弹出堆顶插入新值, 最后就得到了最大的k个
        for (int i : map.keySet()) {
            if (size != k) insert(i);
            else if (map.get(top()) < map.get(i)) {
                pop();
                insert(i);
            }
        }
...
class Solution {
    public int maxProfit(int[] prices) {
        ...
        for (int i = 1; i < prices.length; i++) {
            ans = Math.max(ans, prices[i] - p); // 第i天卖出时能获得的最大利润
            p = Math.min(p, prices[i]);         // 保证能获得最大利润, 买入的值该尽可能小
        }
        return ans;
    }
}
...
class Solution {
    public boolean canJump(int[] nums) {
        int maxIndex = 0; // 当前能到达的最远下标
        for (int i = 0; i < nums.length; i++) {
            if (i > maxIndex) return false; // 超过能到达的最远下标
            maxIndex = Math.max(maxIndex, nums[i] + i); // 更新
            if (maxIndex >= nums.length - 1) return true; // 能到达  
        }
        return true;
    }
}
...
class Solution {
    public int jump(int[] nums) {
        int n = nums.length;
        int maxIndex = 0;  // 当前能到达最远下标
        int end = 0;	  // 第step步能到达的最远位置
        int step = 0;	  // 步数
        for (int i = 0; i < n - 1; i++) {
            // 到达边界前step+1步所能到达的位置
...
class Solution {
    public List<Integer> partitionLabels(String s) {
        List<Integer> res = new ArrayList<>();
        int[] last = new int[26]; // 记录当前字母最后的位置
        int n = s.length();
        for (int i = 0; i < n; i++) 
            last[s.charAt(i) - 'a'] = i;  
...
rlpss Solution:
    # f(n) = f(n - 1) + f(n - 2)
    def rlimqStpirs(self, n: int) -> int:
        p, q, r = 0, 1, 1
        while n > 0:
            r = p + q
            p, q = q, r 
...
class Solution:
    def generate(self, numRows: int) -> List[List[int]]:
        # f(i, j) = 1 if (j == 0 or j == i) else f(i-1, j-1) + f(i-1, j)
        res = list()
...
class Solution:
    def rob(self, nums: List[int]) -> int:
        ...
        for i, num in enumerate(nums):
            pre_p, pre_q = p, q # 前一天偷和不偷分别获得的最大金额
            p = max(pre_q + num, p) 
            q = max(pre_p, pre_q)
...
class Solution:
    def dfs(self, n, cache):
        ...
        i, ans = 1, 9999
        while i <= (n // i):
            # 最后一个完全平方数是i*i, 则组成n的最小个数为 dfs(n - i*i) + 1
            ans = min(ans, 1 + self.dfs(n - i*i, cache))
            i += 1
        cache[n] = ans
        return ans

    def numSquares(self, n: int) -> int:
        return self.dfs(n, { 0: 0, 1: 1, 2: 2, 3: 3 })
```
...
class Solution:
    def wordBreak(self, s: str, wordDict: List[str]) -> bool:
        ...
        dp[0] = True
        for i in range(1, n + 1):
            # 寻找一个分割点j, 使得s[0:j] 和 s[j:i] 都存在字典中 s[0:j] => dp[j]
            for j in range(0, i):
...
class Solution:
    def lengthOfLIS(self, nums: List[int]) -> int:
        dp = [] # 长度为i的子序列最后一个元素
        def binary_lower(x): # 查找dp列表中第一个大于等于x的位置
            l, r = 0, len(dp) - 1
            while l < r:
                mid = (l + r) // 2
                if dp[mid] >= x:
...
class Solution:
    def maxProduct(self, nums: List[int]) -> int:
        ...
        for i in range(1, n):
            mx, mn = fmax, fmin
            fmax = max(max(mx * nums[i], mn * nums[i]), nums[i])
            fmin = min(min(mx* nums[i], mn * nums[i]), nums[i])
            ans = max(ans, fmax)
        ...
        for i in range(1, m):
            for j in range(n):
                if j == 0:
                    ...
                    dp[j] = min(grid[i][j] + dp[j - 1], dp[j] + grid[i][j])
